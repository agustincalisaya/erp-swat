...
- Si `variacion_porcentual_maxima` supera el umbral parametrizado por Dirección: `publicada = false`, `requiere_aprobacion = true`, la versión queda creada pero **no habilitada** para su uso en nuevas `OrdenCompra` hasta que un Supervisor de Compras la apruebe explícitamente vía un endpoint de aprobación (`PATCH /api/proveedores/[id]/lista-precios/[version_id]/aprobar/route.ts`, mismo patrón de homologación de proveedor de la sección 2.2, fuera del detalle de este documento por brevedad pero sujeto a las mismas reglas transaccionales y de evento crítico).
...
**Comportamiento esperado:**
1. Resuelve el conjunto de proveedores candidatos: si se recibe `variante_sku_id`, todos los `Proveedor` con `estado = "HOMOLOGADO"` que tengan un `ListaPrecioItem` para esa variante en su `ListaPrecioVersion` vigente (misma consulta de "versión vigente" de 2.3/3.4); si se recibe `categoria`, todos los `Proveedor` homologados que tengan al menos un `ListaPrecioItem` vigente cuya `VarianteSKU` pertenezca a un `ProductoMaestro` con `categoria` igual al valor recibido (`ListaPrecioItem.variante_sku_id → VarianteSKU.producto_maestro_id → ProductoMaestro.categoria`) — **sin cruzar contra `Proveedor.categorias`** (ver nota de corrección abajo).
...
**Nota de corrección (relevamiento previo a la implementación de HU-H7, categoría de filtro):** la redacción original del punto 1 ("todos los `Proveedor` homologados cuyo array `categorias` contenga ese valor, restringido además a los que tengan al menos un ítem vigente en esa categoría") era ambigua — no aclaraba de qué modelo salía "esa categoría" del ítem — y, en su lectura literal, un relevamiento de solo lectura confirmó que produce falsos negativos contra los propios datos de referencia del proyecto: `proveedorHomologado` del seed (`categorias: ["Textil Táctico", "Calzado"]`, `prisma/seed.ts:456`) queda excluido de sus propias categorías reales de producto (`ProductoMaestro.categoria = "Camisas"` para CAMPOL/CAMTAC, `"Botas"` para BORCEG, vía `VarianteSKU.producto_maestro_id`) porque no hay overlap entre ambos conjuntos de strings. El relevamiento adicional confirmó: (1) `Proveedor.categorias` no tiene ningún uso como restricción de negocio en el código — solo aparece en el ciclo CRUD/UI del propio Proveedor (validación Zod al alta, persistencia, formulario, badge de listado); ningún Route Handler ni servicio lo usa para filtrar o restringir nada; (2) el Documento de Alcance Funcional (§3.1) lo describe como dato puramente descriptivo del legajo comercial, al mismo nivel que datos impositivos y contactos, no como un control operativo; (3) ningún flujo de asignación de `VarianteSKU.proveedor_id` sincroniza ese array contra las categorías reales de los productos que el proveedor efectivamente provee — la única regla aplicada ahí es `estado = HOMOLOGADO`. En consecuencia, el criterio de filtrado por `categoria` se resuelve **exclusivamente** por `ProductoMaestro.categoria`, sin cruzar contra `Proveedor.categorias` — decisión ya reflejada en el punto 1 de arriba.
