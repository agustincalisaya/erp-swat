...
import { ClienteIdConsentimientoSchema, TransicionConsentimientoSchema } from "@/lib/schemas/consentimientos.schema";
import { esAdministradorCrmActivo, PERMISO_GESTIONAR_CONSENTIMIENTO, transicionarConsentimientoCliente } from "@/lib/services/clientes/consentimiento.service";
...
export const POST = withPermission(PERMISO_GESTIONAR_CONSENTIMIENTO,
  async (req: NextRequest, session, rawContext) => {
    ...
    const parsed = TransicionConsentimientoSchema.safeParse(await req.json().catch(() => null));
    ...
    if ((parsed.data.operacion === "EJECUTAR_REVOCACION" || parsed.data.operacion === "RECHAZAR_SOLICITUD" ||
      ...
      return errorJson("FORBIDDEN", "Esta operación requiere el rol Administrador CRM activo", 403);
    }
    try {
      const data = await transicionarConsentimientoCliente(id, parsed.data, session.userId);
      return NextResponse.json({ data, error: null }, { status: 201 });
    } catch (error) {
      if (error instanceof ServiceError) {
        const status = error.code === "CLIENTE_NO_ENCONTRADO" ? 404 : error.code === "FORBIDDEN" ? 403 :
          error.code === "VALIDATION_ERROR" ? 400 : 409;
        return errorJson(error.code, error.message, status);
      }
      console.error("[POST /api/clientes/[id]/consentimientos/transiciones] Error inesperado:", error);
